...
void MM_test() {
  ...
  // ckks_evaluator.decryptor.decrypt(input_cipher, output_plain);
  // // ckks_evaluator.decryptor.decrypt(input_cipher, temp_plain);
  
  // ckks_evaluator.encoder.decode(output_plain, output);
  // // ckks_evaluator.encoder.decode(temp_plain, temp);
  ...
  // cout << endl;
  // // for (auto i = 0; i < 7; i++) {
  // //   cout << fixed << setprecision(5) << temp[i] << ", ";
  // // }
  // // cout << endl;
}
